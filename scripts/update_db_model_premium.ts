...
async function main() {
    console.log("Actualizando modelo Premium de la BD a 'gemini-1.5-flash' para sincronizar con la interfaz...");
    ...
    try {
        ...
        const res = await client.query(`
            ...
            SET "modelPremium" = 'gemini-1.5-flash'
            WHERE id = 'singleton'
        `);
        console.log(`[+] Modelo Premium actualizado a 'gemini-1.5-flash'. Filas afectadas: ${res.rowCount}`);
        client.release();
    } catch (error: any) {
