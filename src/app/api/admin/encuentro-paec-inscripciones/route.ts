...
export async function GET(req: NextRequest) {
    ...
    if (format === "csv") {
        let csv = "CCT,Escuela,Nombre Proyecto,Problemática,Alumno,Grado,CURP,Fecha Registro\n";
        for (const insc of inscripciones) {
            const datos = insc.datos as { nombreProyecto?: string; problematicaEntorno?: string; alumnos?: { nombre: string; grado: string; curp: string }[] };
            const alumnos = datos.alumnos || [];
            for (const al of alumnos) {
                csv += `"${insc.escuela.cct}","${insc.escuela.nombre}","${(datos.nombreProyecto || "").replace(/"/g, '""')}","${(datos.problematicaEntorno || "").replace(/"/g, '""')}","${al.nombre}","${al.grado}","${al.curp}","${insc.createdAt.toISOString().split("T")[0]}"\n`;
            }
        }
        return new NextResponse(csv, {
            headers: {
                "Content-Type": "text/csv; charset=utf-8",
                "Content-Disposition": "attachment; filename=encuentro_paec_2026.csv",
            },
        });
    }
    ...
    const result = inscripciones.map((insc) => {
        ...
        return {
            ...
            problematicaEntorno: datos.problematicaEntorno || "",
