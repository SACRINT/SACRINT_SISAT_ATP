...
export async function GET() {
    ...
    const inscripcion = await prisma.inscripcionOlimpiada.findUnique({
        where: {
    ...
    return NextResponse.json({
        config: config ? { ...config, activo: config.activo && ciclo.activo } : { activo: false, convocatoriaUrl: null },
...
export async function DELETE() {
    ...
    await prisma.inscripcionOlimpiada.deleteMany({
        where: {
            escuelaId: escuela.id,
            cicloEscolarId: ciclo.id,
        },
